...
@router.get('/news/subs/', response_model=list[schemas.Post])
def get_subs_news(
        skip: int = 0,
        limit: int = 3,
        username: schemas.UserCreate = Depends(get_current_user),
        db: Session = Depends(get_db)
):
    profile = get_profile(username, db)
    subs = profile.subscribers
    posts = []
    for sub in subs:
        for post in sub.group.posts:
            posts.append(post)

    posts.sort(key=lambda post: post.datetime_create, reverse=True)
